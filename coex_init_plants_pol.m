...
load(datastring(1,:), 'gridres','init_pla','init_pol','maxpop','e','a','b','w')
u = zeros(gridres);
...
for panel=1:row*col
  ...
  contour(X,Y,u',[0,0],'k');
  contour(X,Y,u',[1,1],'k');
  text(1,5,'\delta_0', 'FontSize',10)
  text(40,10,'\delta_1', 'FontSize',10)
  set(gca,'ticklength',2*get(gca,'ticklength'))
  % Plot labels
  if mod(panel-1,col) == 0
    set(gca,'YTick', tiYs,'YDir','normal', 'FontSize',10)
    ylabel('A(0)      ', 'FontSize',10)
  ...
  if panel > (row-1)*col
    set(gca,'XTick', tiXs, 'YDir','normal')
    xlabel('P_1(0)', 'FontSize',10)
  else
...
print -depsc coex_init_plants_pol.eps
print -dpng coex_init_plants_pol.png
