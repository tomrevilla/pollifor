% REPLICATOR MODEL SIMULATION SCRIPT -- SCENARIO 1
%
...
% pref: preference model case (see odesystem.m)
...
% Population density grid: The sum of plant densities is constrained to be 
% equal to the average of their carrying capacities, so we run across plant 1 
% density. The pollinator density maximum is also the carrying capacity average.
Pop1 = zeros(gridres);
Pop2 = zeros(gridres);
Pop3 = zeros(gridres);
Pref = zeros(gridres);
maxpop = ceil(mean(K));
init_pla = linspace(0, maxpop, gridres);  % Plant range
init_pol = linspace(0, maxpop, gridres);  % Pollinator range
[X,Y] = meshgrid(init_pla, init_pol);
init_prf = zeros(gridres);                % Preference range
for xx = 1:gridres
  x(1:2) = [init_pla(xx), maxpop - init_pla(xx)];
  for yy = 1:gridres
    x(3) = init_pol(yy);
    init_prf(xx,yy) = best_resp_u(x, e.*a, b, w);
  end
end
...
parfor xx = 1:gridres
  ...
  for yy = 1:gridres
        n0 = [init_pla(xx), maxpop-init_pla(xx), init_pol(yy), init_prf(xx,yy)];
